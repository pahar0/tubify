...
spotifyApi.setAccessToken(JSON.parse(fs.readFileSync('./tokens.json')).spotify.access_token)


const getPlaylist = async (req, res, next) => {

    //return res.render('player', { playlistDetails: require('../responseExample.json')})
    const spotifyUrl = req.body.playlist || null
    const youtubeAPI = req.body.youtubeAPI || false

    // TODO: add ==="false" || ==="true"
    if (typeof youtubeAPI !== 'string')
        return res.status(500).send({ error: 'YOUTUBEAPI_INVALID', desc: 'invalid youtube api mode' })

    if (typeof spotifyUrl == null || !validUrl.isUri(spotifyUrl))
        return res.status(500).send({ error: 'URL_INVALID', desc: 'url not valid' })

    const parseSpotifyUrl = url.parse(spotifyUrl, true)

    if (parseSpotifyUrl.pathname == null)
        return res.status(500).send({ error: 'URL_INVALID', desc: 'pathname not valid' })

    const pathnameSplit = parseSpotifyUrl.pathname.split('/')

    if (pathnameSplit[1] == undefined || pathnameSplit[2] == undefined)
        return res.status(500).send({ error: 'URL_INVALID', desc: 'empty playlist type or id' })

    const playlistType = pathnameSplit[1]

    if (playlistType !== "album" && playlistType !== "playlist")
        return res.status(500).send({ error: 'URL_INVALID', desc: 'invalid playlist type or id' })
